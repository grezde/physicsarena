    ...
    let str = `
        ...
        ${ole.map(yr => `
            <li class="year-container year-container-${yr.year}">
                ...
                ${yr.website ? `<p class="year-website year-website-${yr.year}"><a href="${yr.website}">Website</a></p>` : '<!-- No website -->'}
                ${pdata.plevel == 1 ? `
                <h3 class="problems-links problems-links-${yr.year}">
                    <span class="problems-link-p"><a href="/files/${pdata.loc}/${pdata.ol}/${pdata.exam}/${yr.year}-p.pdf">Problems </a></span>
                    <span class="problems-link-s"><a href="/files/${pdata.loc}/${pdata.ol}/${pdata.exam}/${yr.year}-s.pdf">Solutions</a></span>
                </h3>
` : ''}
                ...
                ${yr.problems.map(pr => `
                    <li class="problem-container">    
                        ...
                        <h4 class="problem-links problem-links-${yr.year}-${pr.id}">
                            <span class="problem-link-p"><a href="/files/${pdata.loc}/${pdata.ol}/${pdata.exam}/${yr.year}-${+pr.id+1}-p.pdf">Problem </a></span>
                            <span class="problem-link-s"><a href="/files/${pdata.loc}/${pdata.ol}/${pdata.exam}/${yr.year}-${+pr.id+1}-s.pdf">Solution</a></span>
                        </h4>
` : ''}
                        <p class="problem-topics problem-topics-${yr.year}-${pr.id}">${pr.topics ? pr.topics.split(' ').map(t => `
                            <span class="problem-topic topic-${t}">${getTopicName(pdata, t)}</span> 
`).join('') : ''}</p>
...
<html lang="en">
    ...
    <body>
        <nav>
            ...
            <p><a href="/public/${pdata.loc}/${pdata.ol}/${x.code}">${x.code}</a></p>
            `)}
            <p><a href="/public/${pdata.loc}/${pdata.ol}/ex">ex</a></p>
            <p>${myo.exams.filter(x => x.code != pdata.exam).map(x => x.code)} </p>
            <p class="separator"></p>
            ${odata.filter(o => o.exams.filter(e => e.code == pdata.exam).length > 0).map(o => `
